    ...
    return (
        sortDESC(replies).map((reply, key) => (
            <div key={key} className='mt-2' style={{ display: "flex", paddingLeft: 20 }}>
                ...
                {reply.imageUrl !== null && <img onClick={() => onShowImageModal(reply.imageUrl)} className='p-2 hover' style={{ width: "100%", paddingRight: 7, borderRadius: 15 }} src={reply.imageUrl} alt="" srcSet="" />}
            </div>
        ))
    )
}
